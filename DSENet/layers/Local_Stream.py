...
import numpy as np
from layers.LoRE import LoRE
from layers.LWSA import *
...
class TSTiEncoder(nn.Module):
    def __init__(self, context_window, c_in, local_ws, patch_num, patch_len, max_seq_len=1024,
        ...
        if self.use_lore:
            self.lore = LoRE(patch_len, dropout=dropout)

        self.W_P = nn.Linear(patch_len, d_model)
        self.W_pos = positional_encoding(pe, learn_pe, patch_num, d_model)
        self.dropout = nn.Dropout(dropout)
        self.encoder = TSTEncoder(
            patch_num, d_model, n_heads, d_k=d_k, d_v=d_v, d_ff=d_ff,
            local_ws=local_ws, norm=norm, attn_dropout=attn_dropout,
    ...
    def forward(self, x):
        ...
        n_vars = x.shape[1]
        x = x.permute(0, 1, 3, 2)  # [B, nvars, patch_num, patch_len]

        if self.use_lore:
            x = self.lore(x)

        x = self.W_P(x)  # [B, nvars, patch_num, d_model]

        u = x.reshape(x.shape[0] * x.shape[1], x.shape[2], x.shape[3])
        u = self.dropout(u + self.W_pos)
        z = self.encoder(u)
        z = z.reshape(-1, n_vars, z.shape[-2], z.shape[-1]).permute(0, 1, 3, 2)
        return z
